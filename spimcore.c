...
void Step(void)
{
	/* fetch instruction from memory correct */
	Halt = instruction_fetch(PC,Mem,&instruction);

	if(!Halt)
	{
		/* partition the instruction correct*/
	...
	if(!Halt)
	{
//one of these functions is halting
		/* read_register correct*/
